...
namespace Engine
{
	...
	const GeometryData* MeshAssetManager::GetMesh(const std::filesystem::path& assetUrl)
	{
		auto it = mModelSet.find(assetUrl.generic_string());
		...
		if(LoadMeshAsset(assetUrl))
		{
			return mModelSet[assetUrl.generic_string()].get();
		}

		return nullptr;
	}

	bool MeshAssetManager::LoadMeshAsset(const std::filesystem::path& assetUrl)
	{
		...
		GeometryData& geometry = *(mModelSet[assetUrl.generic_string()] = std::make_unique<GeometryData>());

		processNode(scene->mRootNode, scene, geometry.Meshes);
