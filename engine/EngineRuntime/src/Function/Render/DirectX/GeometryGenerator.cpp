...
namespace Engine
{
	...
	GeometryGenerator::MeshData GeometryGenerator::CreateSphere(float radius, uint32 sliceCount, uint32 stackCount)
	{
		...
		for (uint32 i = 1; i <= stackCount - 1; ++i)
		{
			...
			for (uint32 j = 0; j <= sliceCount; ++j)
			{
				...
				v.TangentU.Normalise();
				//XMVECTOR T = XMLoadFloat3(&v.TangentU);
				//XMStoreFloat3(&v.TangentU, XMVector3Normalize(T));

				v.Position.Normalise();
				//XMVECTOR p = XMLoadFloat3(&v.Position);
				//XMStoreFloat3(&v.Normal, XMVector3Normalize(p));


				v.TexC[0] = theta / Math_TWO_PI;
				v.TexC[1] = phi / Math_PI;
	...
	GeometryGenerator::Vertex GeometryGenerator::MidPoint(const Vertex& v0, const Vertex& v1)
	{
		...
		*/
		...
		v.TangentU = Vector3::Lerp(v0.TangentU, v1.TangentU, 0.5f);
		v.TexC = Vector2::Lerp(v0.TexC, v1.TexC, 0.5f);
	...
	GeometryGenerator::MeshData GeometryGenerator::CreateGeosphere(float radius, uint32 numSubdivisions)
	{
		...
		*/
		...
		for (uint32 i = 0; i < meshData.Vertices.size(); ++i)
		{
			meshData.Vertices[i].Position.Normalise();
			meshData.Vertices[i].Normal = meshData.Vertices[i].Position;
			meshData.Vertices[i].Position *= radius;
			...
			float phi = acosf(meshData.Vertices[i].Position[1] / radius);

			meshData.Vertices[i].TexC[0] = theta / Math_TWO_PI;
			meshData.Vertices[i].TexC[1] = phi / Math_PI;
			...
			meshData.Vertices[i].TangentU.Normalise();
		}
	...
	GeometryGenerator::MeshData GeometryGenerator::CreateCylinder(float bottomRadius, float topRadius, float height, uint32 sliceCount, uint32 stackCount)
	{
		...
		for (uint32 i = 0; i < ringCount; ++i)
		{
			...
			for (uint32 j = 0; j <= sliceCount; ++j)
			{
				...
				vertex.TexC[0] = (float)j / sliceCount;
				vertex.TexC[1] = 1.0f - (float)i / stackCount;
				...
				*/
				vertex.Normal = vertex.TangentU.CrossProduct(bitangent);
	...
	GeometryGenerator::MeshData GeometryGenerator::CreateGrid(float width, float depth, uint32 m, uint32 n)
	{
		...
		for (uint32 i = 0; i < m; ++i)
		{
			...
			for (uint32 j = 0; j < n; ++j)
			{
				...
				meshData.Vertices[i * n + j].TexC[0] = j * du;
				meshData.Vertices[i * n + j].TexC[1] = i * dv;
			}
		}
